...
//PostgresStruct #TODO need to write some comment & need to write different name instead of PostgresStruct
...
func (s PostgresStruct) createTable(tableName string, fieldDataType []string) {
	query := "CREATE TABLE " + tableName + " (" + strings.Join(fieldDataType, ",") + ")"
	s.execQuery(query)
	return
}

func (s PostgresStruct) dropTable(tableName string) {
	query := "DROP TABLE " + tableName
	s.execQuery(query)
	return
}

func (s PostgresStruct) addColumn(tableName string, columnName string, dataType string) {
	query := "ALTER TABLE " + tableName + " ADD " + columnName + " " + dataType
	s.execQuery(query)
	return
}

func (s PostgresStruct) dropColumn(tableName string, columnName string) {
	query := "ALTER TABLE " + tableName + " DROP " + columnName
	s.execQuery(query)
	return
}

func (s PostgresStruct) addIndex(tableName string, indexType string, field []string) {
...
func (s PostgresStruct) dropIndex(tableName string, indexType string, field []string) {
	// #TODO currently indexType is always empty as we don't have a proper way.
...
func (s PostgresStruct) renameTable(oldTableName string, newTableName string) {
